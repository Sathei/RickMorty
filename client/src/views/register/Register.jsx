    ...
    switch (name) {
      ...
      case "password":
        if (state.password === "") {
          ...
          setErrors({ ...errors, password: "Must have at least 3 characters" });
        } else {
      ...
      {!loaded ? (
        ...
        <div className="container cont-register flex flex-col p-2 justify-center mx-auto w-1/3 h-4/6 rounded-xl text-white">
          <form onSubmit={handleSubmit} className="flex flex-col items-center">
            ...
            <br />
            <button className="btn-sign bg-green-600 w-1/5 p-1 rounded-lg" disabled={disableFunction()} type="submit">Sign up</button>
          </form>
        </div>
      )}
    </div>
